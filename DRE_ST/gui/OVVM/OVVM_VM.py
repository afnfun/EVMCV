...
class VM():
	def __init__(self):
		...
		if (self.Options_List.ns1_EMLHeader.ns1_OfficialStatusDetail.ns1_OfficialStatus.PCDATA != 'approved'):
				return 0 , 'error!! Options_List630.xml is not approved, Contact election official'
		self.commands={'1':'connect','2':'close','3':'report','4':'activate','5':'votercast','6':'officercast','7':'officercancel','8':'select','9':'cancel','A':'writein','B':'reset','C':'review','D':'reason'}
		self.Event_Name=None
		self.Event_ID=None
		self.Election_Name=None
		self.Election_Start_Date=None
		self.Election_End_Date=None
		self.Polling_Place_Name=None
		self.Polling_Place_ID=None
		self.Polling_Station_ID=None
		self.Vm_ID='v001'
		self.Ballot_ID=None
		self.VMSetting_ID=None
		self.contests=[]
		self.vote_options=[]
		self.vote_options_writein=[]
		self.add_private_log=False
		self._Vote_ID=0
		self._last_command=None     #value between 0-15
		self._last_sequence = '-1'
		self._last_command_time =''
		self._current_command=None  #value between 0-15
	...
	def check_logical_sequence(self,command):
		...
		if (command=='reason'):
			...
			error= 'Error(103)!! Invalid command sequence. The command '+command+' cannot come after ('+str(self._last_command)+') that arrived recently at ('+str(self._last_command_time)+').\n'
			error= error + message
			self.add_log(error)
			return False, error
	...
	def _check_message_sequence(self):
		...
		if seq==1: #if the difference is 1 then it is ok.
	...
	def get_VMSetting_Information(self,VMSetting_ID):
		...
		for VMSet in self.VMS.ns1_VmSettings:
			for Set in VMSet.ns1_Settings:
				if (Set.IdNumber==VMSetting_ID):
					Setting_Exist=True
					Event_Id=str(VMSet.ns1_EventIdentifier.IdNumber)
					Polling_Place_ID=str(Set.ns1_PollingPlaceID.PCDATA)
					Polling_Station_ID=str(Set.ns1_PollingStationID.PCDATA)
					Ballot_ID=str(Set.ns1_BallotIdentifireID.PCDATA)
		...
		if (Setting_Exist==False): #if there was no match
			error='Error (105)!! The received  VMSetting identifeir is not included in the VMSetting.xml file, machine must exit !'
			self.add_log(error)
			return 0, error
		else:
				...
				if (contest_info!=0):
					...
					for vote in self.vote_options[index_contest]:
						...
						if vote[6:8]=='00': # if vote option ID for writein
							vote_summary+=	'		You have select as a write in : ' + self.vote_options_writein[index_contest] + '\n'
						else: # if it is not writein
							option_info= self.get_Option_Information(self.Ballot_ID,vote)
							if option_info[0]!=0:
								vote_summary+=	'		You have select : ' + option_info[2] +  '\n'
							else:
								vote_summary+=option_info[1]
	...
	def votes_summary(self,VMSetting_ID):
		...
		print self.vote_options_writein
		...
		for castvote in xml_votes_list:
			...
			if vote_instance.ns1_CastVote.ns1_EventIdentifier.IdNumber!=Event_Id:
				...
				continue
			if str(vote_instance.ns1_CastVote.ns1_AuditInformation.ns1_ProcessingUnits.ns1_OriginatingDevice.ns1_IdValue.PCDATA)!=str(Polling_Place_ID)+str(Polling_Station_ID):
				print str(vote_instance.ns1_CastVote.ns1_AuditInformation.ns1_ProcessingUnits.ns1_OriginatingDevice.ns1_IdValue.PCDATA),str(Polling_Place_ID)+str(Polling_Station_ID)
				...
				for Cont in self.contests:
					...
					if ElectionType!='ordered': #the type of Cont is not ordered
						print 'This is not ordered contest'
					...
					if ElectionType=='ordered': #the type of Cont is ordered
						...
						for option_id in votes_order:
							...
							if option_id in self.vote_options[index].keys():
								...
								if option_id[6:8]=='00':
									if Contest_ID in ordered_writein.keys():
										if writein in ordered_writein[Contest_ID].keys():
											if int(option_index) in ordered_writein[Contest_ID][writein].keys():
												value = int(ordered_writein[Contest_ID][writein][option_index])+1
												ordered_writein[Contest_ID][writein][option_index]=str(value)
											else:
												ordered_writein[Contest_ID][writein].update({option_index:1})
										else:
											ordered_writein[Contest_ID].update({writein:{option_index:1}})
									#else:
								...
								if option_id[6:8]=='00':
									self.vote_options[index].update({option_id:{option_index:1}})
									ordered_writein.update({Contest_ID:{writein:{option_index:1}}})
		#
		#
