...
#import socket
#
#HOST = '192.168.31.255'                 # Symbolic name meaning the local host
#HOST='<broadcast>'
...
class Ballot:
	...
	def load( self):
		...
		headParser = re.compile("([0-9]+) ([0-9]+) ([tf]) ([tf])\n") # define the structure of headparser (minVotes maxVotes ordered (t or f)  couples (t or f)  \n)
		parser = re.compile("([0-9]+) ([0-9]+)\n")                   # define the structure of headparser (X Y \n)
		...
		try:
			...
			while 1:
				...
				if match is None: break # if there is error in parsing, do nothing
			...
			for event in pygame.event.get():
				...
				if event.type is KEYDOWN:
					...
					if ((pos[0] >= 940 and pos[0] <= 1203 and
						pos[1] >= 860 and pos[1] <= 946) or (pos[0] >= 1178 and pos[0] <= 1244 and
						pos[1] >= 14 and pos[1] <= 53)): # cast vote
						# Verification
...
def set_video_mode():
	...
	screen = pygame.display.set_mode( (screen_width, screen_height),
		0)
	pygame.display.set_caption('OVC: ICT4G Version')
...
def intro_screen():
	...
	screen.blit( continue_image, (0,0))
	pygame.display.update()
	now1 = datetime.datetime.now()
	while 1:
		now2 = datetime.datetime.now()
